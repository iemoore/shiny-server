...
observeEvent(input$numKeys,{
  ...
  if(a==3){
    ...
    b <- c(6.251, -75.558, 6.244, -75.572)
    b <- input$map_bounds %>% as.numeric()
    ...
    if(l1>0 && l1<1000){
      ...
      leafletProxy("map",data = df1) %>%
        clearShapes() %>% 
        addPolygons(group = "line",color = "black",fill = "transparent")
      
    } 
    
    if(l1>1000){
      
      shinyalert("Whooooa!", ">1000 items available for this area", type = "error")
      
    }
    if(l1==0){
      
      shinyalert("Oops!", "No data available for this area", type = "error")
